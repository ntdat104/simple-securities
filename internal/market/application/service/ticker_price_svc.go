...
// TickerPriceSvc defines the interface for fetching symbol price tickers.
type TickerPriceSvc interface {
	Execute(ctx context.Context, symbol *string, symbols *[]string, opts ...client.RequestOption) ([]*response.TickerPriceResponse, error)
}

type tickerPriceSvc struct {
	c  *client.Client
	sf singleflight.Group
}

func NewTickerPriceSvc(c *client.Client) TickerPriceSvc {
	return &tickerPriceSvc{
		c: c,
	}
}

func (s *tickerPriceSvc) Execute(
	ctx context.Context,
	symbol *string,
	symbols *[]string,
	opts ...client.RequestOption,
) (res []*response.TickerPriceResponse, err error) {
	...
	if len(raw) > 0 && raw[0] == '[' {
		res = make([]*response.TickerPriceResponse, 0)
		...
		singleRes := new(response.TickerPriceResponse)

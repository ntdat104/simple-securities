...
func TestEncrypt(t *testing.T) {
	...
	for _, test := range caesarTestData {
		t.Run(test.description, func(t *testing.T) {
			actual := CaesarEncrypt(test.input, test.key)
...
func TestDecrypt(t *testing.T) {
	...
	for _, test := range caesarTestData {
		t.Run(test.description, func(t *testing.T) {
			actual := CaesarDecrypt(test.input, test.key)
			if actual != test.expected {
				t.Logf("FAIL: %s", test.description)
				t.Fatalf("With input string '%s' and key '%d' was expecting '%s' but actual was '%s'",
					test.input, test.key, test.expected, actual)
...
func Example() {
	...
	encryptedText := CaesarEncrypt(input, key)
...
func FuzzCaesar(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, input string) {
		key := rnd.Intn(26)
		if result := CaesarDecrypt(CaesarEncrypt(input, key), key); result != input {
